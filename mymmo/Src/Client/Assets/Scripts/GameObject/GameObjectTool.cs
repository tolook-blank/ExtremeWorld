...
{
    //逻辑坐标以cm为单位，世界坐标以m为单位
    public static Vector3 LogicToWorld(NVector3 vector)//逻辑坐标到世界坐标 需要除以100
    {
        return new Vector3(vector.X / 100f, vector.Z / 100f, vector.Y / 100f);//Unity中 世界坐标Y轴向上，而逻辑坐标Z轴向上
    }
    ...
    //逻辑坐标以cm为单位，世界坐标以m为单位
    public static NVector3 WorldToLogicN(Vector3 vector)// 世界坐标 到 逻辑坐标,需要乘以100
    {
        return new NVector3()
        {
